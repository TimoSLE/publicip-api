...
.PHONY: build
build:
	@go build -ldflags '${LD_FLAGS}' -o "${OUTPUT_PREFIX}-${GOOS}-${GOARCH}${OUTPUT_SUFFIX}" main.go

# build go application for docker usage
.PHONY: build-docker
build-docker:
	@CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -ldflags '${LD_FLAGS}' -o "${OUTPUT_PREFIX}-docker" main.go

# installs and formats the project with the built-in Golang tool
install:
	@go install -ldflags '${LD_FLAGS}' main.go